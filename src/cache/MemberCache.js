...
class MemberCache extends BaseCache {
  ...
  async get(id, guildId = this.boundGuild) {
    ...
    return new MemberCache(
      this.storageEngine,
      this.user.bindUserId(member.id),
      member
    );
  }
  ...
  async filter(fn, guildId = this.boundGuild, ids = null) {
    let members = await this.storageEngine.filter(
      fn,
      ids,
    ...
    return members.map(m =>
      new MemberCache(
        this.storageEngine,
        this.user.bindUserId(m.id),
        m
      ).bindGuild(this.boundGuild)
    );
  }
