...
class EventProcessor extends EventEmitter {
    ...
    switch (event.t) {
      ...
      case "GUILD_DELETE":
        this.emit(
          "debug",
          `Guild ${event.d.id} ${
            event.d.unavailable ? "is unavailable" : "was removed"
          }`
        );
      ...
      case "GUILD_MEMBERS_CHUNK": {
        ...
        for (let member of event.d.members) {
          guildMemberChunkPromises.push(
            this.memberCache.update(member.user.id, event.d.guild_id, member)
          );
        }
        ...
        this.emit(
          "debug",
          `Cached ${
            guildMemberChunkPromises.length
          } Members from Guild Member Chunk`
        );
        break;
      }
      case "USER_UPDATE":
      ...
      case "GUILD_EMOJIS_UPDATE": {
        let oldEmotes = await this.emojiCache.filter(
          () => true,
          event.d.guild_id
        );
  ...
  handlePresenceUpdate(presenceEvent) {
    ...
    if (this.presenceQueue[presenceEvent.user.id]) {
      this.presenceQueue[presenceEvent.user.id] = Object.assign(
  ...
  async processReady(readyEvent) {
    ...
    updates.push(
      this.userCache.update(readyEvent.d.user.id, readyEvent.d.user)
    );
  ...
  async onChannelCreate(channelCreateEvent) {
    switch (channelCreateEvent.d.type) {
      ...
      case 2:
      case 4:
        await this.channelMapCache.update(
          channelCreateEvent.d.guild_id,
          [channelCreateEvent.d.id],
          "guild"
        );
        // this.emit('debug', `Caching guild channel ${channelCreateEvent.d.id}`);
        return this.channelCache.update(
          channelCreateEvent.d.id,
          channelCreateEvent.d
        );
      default:
        break;
    }
    if (channelCreateEvent.d.type === 1) {
      if (
        !channelCreateEvent.d.recipients ||
        channelCreateEvent.d.recipients.length === 0
      ) {
        this.emit(
          "debug",
          `Empty Recipients array for dm ${channelCreateEvent.d.id}`
        );
        return;
      }
      // this.emit('debug', `Caching dm channel ${channelCreateEvent.d.id}`);
      await this.channelMapCache.update(
        channelCreateEvent.d.recipients[0].id,
        [channelCreateEvent.d.id],
        "user"
      );
      return this.channelCache.update(
  ...
  async onChannelDelete(channelDeleteEvent) {
    switch (channelDeleteEvent.d.type) {
      ...
      case 2:
        await this.channelMapCache.update(
          channelDeleteEvent.d.guild_id,
          [channelDeleteEvent.d.id],
          "guild",
          true
        );
    ...
    if (channelDeleteEvent.d.type === 1) {
      await this.channelMapCache.update(
        channelDeleteEvent.d.recipients[0].id,
        [channelDeleteEvent.d.id],
        "user",
        true
      );
  ...
  async flushQueue() {
    ...
    this.emit(
      "debug",
      `Flushed presence update queue with ${
        presenceUpdatePromises.length
      } updates`
    );
  }
}
